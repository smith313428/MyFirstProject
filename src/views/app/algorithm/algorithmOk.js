...
const Algorithm = (props) => {
    ...
    for (let i = 0; i <= days.length; i++) {
      ...
      if(i<= days.length-1){
        ...
        if (mode==4){
          ...
          result=Algorithm7(newDays[i],i+1,true,true)
        }else  if(mode==6){
  ...
  const Algorithm7=(dayData,id,long,short)=>{
    ...
    let longPositionObj={}
    let longStartPosition=0
    let longOrderAmount=0
    let longCurPosition=0
    let longProfit=0
    let longDifference=0
    let longCurLoss=0
    let longProfitArray=[]
    let longCurLossArray=[]

    let shortPositions=[]
    let shortPositionObj={}
    let shortStartPosition=0
    let shortOrderAmount=0
    let shortCurPosition=0
    let shortProfit=0
    let shortDifference=0
    let shortCurLoss=0
    ...
    let dayLongTotalArray=[]
    let dayShortTotalArray=[]
    let shortCurLossArray=[]
    let dayTotalArray=[]
    let shortProfitArray=[]
    let dayTotal=0
    let dayLongTotal=0
    let dayShortTotal=0

    let realPrice=0
    let longOrdering=false
    priceData.map((item, idx)=>{
      if (shortOrderAmount==0){
        shortStartPosition=shortStartPosition!=item?item:shortStartPosition
        shortCurPosition=item
        shortOrderAmount=unitAmount
        shortPositionObj={'amount':unitAmount,'price':item}
        shortPositions.push(shortPositionObj)
      }
      if (longOrderAmount==0){
        longStartPosition=longStartPosition!=item?item:longStartPosition
        longCurPosition=item
        longOrderAmount=unitAmount
        longPositionObj={'amount':unitAmount,'price':item}
        longPositions.push(longPositionObj)
      }
      else{
        longDifference=item-longPositions[longPositions.length-1].price
        // console.log('longData:',idx,item,longCurPosition,longOrderAmount,longDifference)
        // console.log('longData____1:',idx,item,longOrderAmount,shortOrderAmount,longDifference,shortPositions[shortPositions.length-1].price-item)
        if(longDifference>=profitVolume){
          longProfit=longProfit+(item-longCurPosition)*(unitAmount/20000)
          // console.log('longData_profit:',idx,item,longCurPosition,longOrderAmount,longDifference,longProfit)
          longOrderAmount=longOrderAmount-unitAmount
          longPositions.pop()
          if(longPositions.length==0){
            longStartPosition=item
            longCurPosition=item
            longOrderAmount=unitAmount
            longPositionObj={'amount':unitAmount,'price':item}
            longPositions.push(longPositionObj)
          }
          // console.log('longProfit:',longProfit)
        }
        else if((longOrderAmount<unitAmount*10)&&((longDifference<=-profitVolume&&longOrderAmount<shortOrderAmount)||(longDifference<=-profitVolume&&longOrderAmount==shortOrderAmount&&longDifference<shortPositions[shortPositions.length-1].price-item)&&shortPositions[shortPositions.length-1].price<item)){
          // console.log('longData____:',idx,item,item-longCurPosition,longCurPosition,unitAmount/(longOrderAmount+unitAmount),(longCurPosition-item)*(unitAmount/(longOrderAmount+unitAmount)))
          longCurPosition=longCurPosition-((longCurPosition-item)*(unitAmount/(longOrderAmount+unitAmount)))
          longOrderAmount=longOrderAmount+unitAmount
          longPositionObj={'amount':unitAmount,'price':item}
          longPositions.push(longPositionObj)
          // console.log('longPositions:',longPositionObj.price)
          longOrdering=true
        }
      }
      longCurLoss=((longCurPosition-item)*((longOrderAmount)/20000))

      if (shortOrderAmount==0){
      }
      else{
        shortDifference=shortPositions[shortPositions.length-1].price-item
        // console.log('shortData:',idx,item,shortCurPosition,shortOrderAmount,shortDifference,shortProfit)
        // console.log('shortData____1:',idx,item,longOrderAmount,shortOrderAmount,shortDifference,longDifference,longPositions[longPositions.length-1].price)
        if(shortDifference>=profitVolume){
          shortProfit=shortProfit+(shortCurPosition-item)*(unitAmount/20000)
          shortOrderAmount=shortOrderAmount-unitAmount
          shortPositions.pop()
          if(shortPositions.length==0){
            shortStartPosition=item
            shortCurPosition=item
            shortOrderAmount=unitAmount
            shortPositionObj={'amount':unitAmount,'price':item}
            shortPositions.push(shortPositionObj)
        ...
        else if((shortOrderAmount<unitAmount*10)&&((!longOrdering&&shortDifference<=-profitVolume&&longOrderAmount>shortOrderAmount)||(shortDifference<=-profitVolume&&longOrderAmount==shortOrderAmount&&shortDifference<longDifference)&&longPositions[longPositions.length-1].price>item)){
          // console.log('shortData____:',idx,item,item-shortCurPosition,shortCurPosition,unitAmount/(shortOrderAmount+unitAmount),shortDifference*(unitAmount/(shortOrderAmount+unitAmount)))
          shortCurPosition=shortCurPosition+((item-shortCurPosition)*(unitAmount/(shortOrderAmount+unitAmount)))
          shortOrderAmount=shortOrderAmount+unitAmount
          shortPositionObj={'amount':unitAmount,'price':item}
          shortPositions.push(shortPositionObj)
        }
        if(longOrdering){
          longOrdering=!longOrdering
        }
      }
      shortCurLoss=((item-shortCurPosition)*((shortOrderAmount)/20000))
      realPrice=item

      dayTotal=(longProfit+shortProfit-longCurLoss-shortCurLoss)
      dayLongTotal=(longProfit-longCurLoss)
      dayShortTotal=(shortProfit-shortCurLoss)
      let periodN=(24*(60/(5*period.value))).toFixed(0)
      // let periodN=1
      if(((idx)%periodN)==0){
        shortCurLossArray.push(-shortCurLoss.toFixed(0))
        shortProfitArray.push(shortProfit.toFixed(0))
        longCurLossArray.push(-longCurLoss.toFixed(0))
        longProfitArray.push(longProfit.toFixed(0))
        dayTotalArray.push(dayTotal.toFixed(0))
        dayLongTotalArray.push(dayLongTotal.toFixed(0))
        dayShortTotalArray.push(dayShortTotal.toFixed(0))
      }
  ...
  // console.log('seriesTmp:',seriesTmp)
    ...
    result = {
      'id':id,
      'longProfit':longProfit.toFixed(0),
      'longLoss':-longCurLoss.toFixed(0),
      // 'longCurPosition':longCurLossTmp>0?longCurPosition.toFixed(0):0,
      'longCurPosition':longCurPosition.toFixed(0),
      'longLossActive':longCurLoss>0?true:false,
      'longRealPosition':realPrice,
      'longAmount':longOrderAmount.toFixed(0),
      'shortProfit':shortProfit.toFixed(0),
      'shortLoss':-shortCurLoss.toFixed(0),
      // 'shortCurPosition':shortCurLossTmp>0?shortCurPosition.toFixed(0):0,
      'shortCurPosition':shortCurPosition.toFixed(0),
      'shortLossActive':shortCurLoss>0?true:false,
      'shortRealPosition':realPrice,
      'shortAmount':shortOrderAmount?.toFixed(0),
      'dayTotal':(longProfit-longCurLoss+shortProfit-shortCurLoss).toFixed(0),
      'realPriceMin':0,
      'realPriceMax':0,
    }
    return result
  }
  const New_Algorithm6=(dayData,id,long,short)=>{
    console.log('dayData:',dayData)
    // console.log('days:',days)
    let longVolume=0
    let longCurPosition=0
    ...
    for (let i = 0; i <= priceData.length-2; i++) {
      ...
      longVolume=priceData[timeIdx]-longStartPosition
      longOneVolume=priceData[timeIdx]-longPositionsTmp[longPositionsTmp.length-1]
      if(unitAmount*longPositionsTmp.length<accountAmount){
        if(longVolume<=(-longPriceVolume*(longPositionsTmp.length))&&(longOneVolume<-longPriceVolume)){
        // if(longVolume<=-longPriceVolume*(longPositionsTmp.length)){
          longCurPosition=longCurPosition-((longCurPosition-priceData[timeIdx])*(unitAmount/(longAmountTmp+unitAmount)))
          longPositionsTmp.push(priceData[timeIdx])
      ...
      // console.log('longNum, realValue,curPosition,longVolume,amount, longOneVolume...',i,(i/288).toFixed(0),priceData[timeIdx],longCurPosition,longVolume,longAmountTmp,longOneVolume)
      longDifference = 0
      for (let j =1; j <= longPositionsTmp.length; j++) {
        longDifference = priceData[timeIdx] -longPositionsTmp[longPositionsTmp.length-j]
        if(longDifference>=profitVolume){
      ...
      shortVolume=priceData[timeIdx]-shortStartPosition
      shortOneVolume=priceData[timeIdx]-shortPositionsTmp[shortPositionsTmp.length-1]
      if(unitAmount*shortPositionsTmp.length<accountAmount){
        // if((shortVolume>=priceVolume*(shortPositionsTmp.length))&&(shortOneVolume>priceVolume)){
        if(shortVolume>=priceVolume*(shortPositionsTmp.length)){
          shortCurPosition=shortCurPosition+((priceData[timeIdx]-shortCurPosition)*(unitAmount/(shortAmountTmp+unitAmount)))
          shortPositionsTmp.push(priceData[timeIdx])
          // console.log('shortPositionsTmp:',i,shortPositionsTmp.length)
      ...
      // console.log('shortNum, realValue, shortCurPosition,amount, shortOneVolume...',i,(i/288).toFixed(0),priceData[timeIdx],shortCurPosition,shortAmountTmp,shortOneVolume)
      shortDifference = 0
      for (let j =1; j <= shortPositionsTmp.length; j++) {
        shortDifference = shortPositionsTmp[shortPositionsTmp.length-j]-priceData[timeIdx]
        if(shortDifference>=profitVolume){
  ...
  const Algorithm6_real=(dayData,id,long,short)=>{
    console.log('dayData:',dayData)
    console.log('days:',days)
    let longVolume=0
    let longCurPosition=0
